...
Phase 6 is not implemented yet. The current enhanced model uses `cmfrec.CMF` with `lambda_`, `w_main`, and `w_user`, which handles side-information reconstruction but does not expose a project-level hook for adding an arbitrary edge-wise graph regularizer of the form needed here. The practical path is to build a custom CMF training method, then keep the existing cmfrec model as the Phase 5 baseline.
...
**Strategy.** Clone the cmfrec GitHub repository into `vendor/cmfrec`, install it in editable mode, and modify three source files:
...
### Step 2: Clone and Install cmfrec from Source

```bash
# from the project root
git clone https://github.com/david-cortes/cmfrec vendor/cmfrec
cd vendor/cmfrec
pip install -e . --no-build-isolation
```

Pin the commit that corresponds to version 3.5.1.post13 so the vendor copy stays deterministic. The editable install makes the compiled `.so` files live in `vendor/cmfrec/cmfrec/` and the conda environment's `cmfrec` import resolves to that directory.
 ...
 *   Penalty  = (lambda_social/2) * sum_{(u,v) in E} w_uv * ||a_u - a_v||^2
 *   Gradient: grad_A[u] += lambda_social * w_uv * (a_u - a_v)        *
 *             grad_A[v] += lambda_social * w_uv * (a_v - a_u)        *
 * Only k+k_main components (rating-relevant dimensions) are          *
 * regularized. k_user components (attribute-only) are left free.     *
 * ------------------------------------------------------------------ */
if (data->lambda_social != 0. && data->social_nnz > 0
    && data->social_row != NULL)
{
    ldouble_safe f_social = 0;
    int_t k_active = k + k_main;   /* rating-relevant latent dimensions */
    for (size_t ix = 0; ix < data->social_nnz; ix++) {
        int_t  u   = data->social_row[ix];
        int_t  v   = data->social_col[ix];
        real_t wuv = data->social_val[ix];
        if (u >= m || v >= m) continue;       /* bounds safety */
        ...
        /* f contribution: (1/2) * w_uv * ||a_u - a_v||^2
           = (1/2) * w_uv * (||a_u||^2 + ||a_v||^2 - 2 a_u.a_v)       */
        ...
        /* gradient for a_u: lambda_social * w_uv * (a_u - a_v) */
        cblas_taxpy(k_active,  data->lambda_social * wuv, Au, 1, gAu, 1);
        cblas_taxpy(k_active, -data->lambda_social * wuv, Av, 1, gAu, 1);
        /* gradient for a_v: lambda_social * w_uv * (a_v - a_u) */
        cblas_taxpy(k_active,  data->lambda_social * wuv, Av, 1, gAv, 1);
        cblas_taxpy(k_active, -data->lambda_social * wuv, Au, 1, gAv, 1);
    }
    /* Consistent with cmfrec convention: f = (1/2) * penalty             */
    f += (real_t)((data->lambda_social / 2.) * f_social);
}
```

**Why `(lambda_social / 2.)`?**
cmfrec stores `f = (lambda/2) * ||A||^2` for its own L2 term, so the gradient is `lambda * A` (not `lambda/2 * A`). The same factor-of-2 convention applies here: storing `(lambda_social/2) * sum w||a_u-a_v||^2` in f gives a gradient of `lambda_social * w * (a_u - a_v)`, consistent with L-BFGS receiving a gradient that is the true gradient of the stored objective.
...
Verify the import resolves to the vendor copy:

```python
import cmfrec, inspect
print(inspect.getfile(cmfrec))
# should be .../vendor/cmfrec/cmfrec/__init__.py
```
...
```python
from cmfrec import CMF  # resolves to vendor/cmfrec
...
5. `SocialRegularizedCMF.predict` returns finite predictions with the same shape as the input arrays.
6. Setting `lambda_social = 0` gives behavior close to the same custom CMF without graph regularization.

## First Milestone

The first implementation milestone should be intentionally narrow:

1. Implement custom CMF with rating loss, user-attribute loss, L2 regularization, and optional Laplacian regularization.
2. Run it with `lambda_social = 0` to validate the training loop.
3. Run it with uniform graph regularization.
4. Add boundary-aware weights only after the uniform regularizer improves or at least behaves stably.

This keeps the research claim clean: first prove that the custom model is a valid replacement for cmfrec in this context, then test whether h-tilde changes the latent geometry in a useful way.

## Main Risk

The most important risk is not coding complexity; it is experimental attribution. If the custom model differs from cmfrec and also adds social regularization at the same time, an improvement could come from either source. That is why `lambda_social = 0` and uniform-social baselines are required before claiming that boundary-guided regularization helped.
