...
def plot_ranking_metrics_per_alpha(
    ...
    """
    Four-panel line plot of ranking metrics (NDCG@K, Precision@K, Recall@K, MRR)
    vs alpha for one diffusion model.

    Reads ``inferred_edges_<short>.csv`` which must contain the ranking-metric
    columns written by :func:`recommender.enhanced._save_rmses` when
    ``compute_ranking=True`` was used.

    Args:
        model_name: Diffusion model name (``"exponential"``, ``"powerlaw"``,
                    ``"rayleigh"``).
        save_plot:  Write PNG to ``plots/models/``.
        figsize:    Figure size.
...
def plot_ranking_metrics_comparison(
    ...
    Each bar group represents one ranking metric; one bar per model.  Only
    metrics that have been computed (i.e. not all-NaN in the CSV) are included.
    ...
    # Only include metrics present in at least one model
    present_metrics = [
        (col, label, colour)
        for col, label, colour in _RANKING_METRICS
...
if __name__ == "__main__":
    ...
    if _do_all or "ranking" in _plots:
        print("\n--- Ranking metrics vs Alpha (per model) ---")
        for _model_name in _model_list:
            plot_ranking_metrics_per_alpha(
                _model_name, save_plot=_save, dataset=_dataset
            )

    if _do_all or "ranking-comparison" in _plots:
        print("\n--- Ranking metrics comparison (all models) ---")
        plot_ranking_metrics_comparison(save_plot=_save, dataset=_dataset)
